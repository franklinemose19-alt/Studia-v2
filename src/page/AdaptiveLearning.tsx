...
export default function AdaptiveLearning() {
  ...
  useEffect(() => {
    ...
    try { results = JSON.parse(localStorage.getItem('quizResults') || '[]') } catch { results = [] }

    // Oldest first, so "improvement" compares first attempt vs latest
    ...
    if (results.length > 1) {
      const first = (results[0].score / results[0].total) * 100
      const latest = (results[results.length - 1].score / results[results.length - 1].total) * 100
  ...
  const sortedHistory = [...quizResults].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          ...
          {quizResults.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 border border-gray-200 text-center">
              <Zap className="mx-auto text-indigo-premium mb-4" size={48} />
              <p className="text-navy font-semibold mb-2">No quizzes taken yet</p>
              <p className="text-gray-600 mb-6">Take a quiz and your performance insights will appear here automatically.</p>
              <button onClick={() => navigate('/quiz')} className="bg-indigo-premium text-white px-6 py-3 rounded-lg hover:bg-purple-premium transition font-medium">
                Go to Quiz
              </button>
            </div>
          ) : (
            <>
              <div className="grid md:grid-cols-3 gap-6">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl p-6 border border-gray-200">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl p-6 border border-gray-200">
                  <div className="flex items-center gap-3 mb-4">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl p-6 border border-gray-200">
                  <div className="flex items-center gap-3 mb-4">
                ...
                {weakTopics.length > 0 ? (
                  <div className="space-y-3">
                    {weakTopics.map((topic, i) => (
                      <motion.div key={topic.topic} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                        className={`rounded-xl p-4 border ${
                          topic.failureRate > 50 ? 'bg-red-50 border-red-200' : topic.failureRate > 25 ? 'bg-yellow-50 border-yellow-200' : 'bg-blue-50 border-blue-200'
                        }`}>
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <p className="font-sora font-bold text-navy mb-1">{topic.topic}</p>
                            <p className="text-sm text-gray-600">{topic.attempts} attempts • Last: {topic.lastAttempt}</p>
                          </div>
                          <div className="text-right">
                            <p className={`font-bold text-2xl ${topic.failureRate > 50 ? 'text-red-600' : topic.failureRate > 25 ? 'text-yellow-600' : 'text-blue-600'}`}>
                              {topic.failureRate}%
                            </p>
                  ...
                  {recommendations.length > 0 ? (
                    recommendations.map((rec, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                        className={`rounded-xl p-4 border flex items-start gap-4 ${rec.priority === 'high' ? 'bg-red-50 border-red-200' : 'bg-yellow-50 border-yellow-200'}`}>
                        <AlertCircle className={rec.priority === 'high' ? 'text-red-600' : 'text-yellow-600'} size={24} />
                        <div className="flex-1">
                          <p className="font-sora font-bold text-navy">{rec.action}</p>
                          <p className="text-sm text-gray-600 mt-1">
                            {rec.priority === 'high' ? 'High priority - Review this topic immediately' : 'Medium priority - Schedule revision soon'}
                          </p>
                        </div>
                        <button onClick={() => navigate('/quiz')} className="bg-indigo-premium text-white px-4 py-2 rounded-lg hover:bg-purple-premium transition text-sm font-medium whitespace-nowrap">
                          Study Now
                        </button>
                      </motion.div>
                    ))
                  ...
                  {sortedHistory.map((result, i) => (
                    <motion.div key={result.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="font-bold text-2xl text-indigo-premium">{Math.round((result.score / result.total) * 100)}%</p>
                          <p className="text-sm text-gray-500">{result.score} / {result.total}</p>
                        </div>
                      </div>
                    </motion.div>
                  ))}
