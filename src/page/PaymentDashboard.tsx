...
import { ArrowLeft, RefreshCw, CheckCircle, Clock, AlertCircle, Copy, Share2, Trophy, Users, Sparkles } from 'lucide-react'
...
const REFERRAL_MINUTES = 5
const AMBASSADOR_THRESHOLD = 100
...
export default function PaymentDashboard() {
  ...
  const fetchPayments = async () => {
    ...
    try {
      const res = await authFetch(`/api/payments?userId=${userIdRef.current}`)
  ...
  const loadReferralData = async (userId: string) => {
    ...
    try {
      const genRes = await authFetch('/api/referral', {
        method: 'POST',
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: "I'm using STUDIA to ace my exams — record lectures, get AI notes & quizzes instantly. Sign up with my link and we both get bonus AI minutes!",
          url: referralLink,
        })
      } catch {
  ...
  const minutesEarned = verifiedCount * REFERRAL_MINUTES
  const ambassadorProgress = Math.min(100, Math.round((verifiedCount / AMBASSADOR_THRESHOLD) * 100))
          ...
          {tab === 'payments' ? (
            ...
            <>
              <div>
                ...
                <p className="text-[#8B97B5]">Invite fellow students — every verified referral earns both of you 5 AI minutes.</p>
              </div>
              ...
              {referralLoading ? (
                ...
                <>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="bg-surface-elevated border border-white/5 rounded-2xl p-6">
                      <div className="flex items-center gap-2 mb-2">
                        <Users size={16} className="text-brand-blue" />
                        <p className="text-sm text-[#8B97B5]">Friends Invited</p>
                      </div>
                      <p className="font-sora font-bold text-3xl text-white">{verifiedCount}</p>
                    </div>
                    <div className="bg-surface-elevated border border-white/5 rounded-2xl p-6">
                      <div className="flex items-center gap-2 mb-2">
                        <Sparkles size={16} className="text-mint" />
                        <p className="text-sm text-[#8B97B5]">Minutes Earned</p>
                      </div>
                      <p className="font-sora font-bold text-3xl text-white">{minutesEarned}</p>
                    </div>
                  </div>
                  ...
                  {!isAmbassador && (
                    <div className="bg-surface-elevated border border-white/5 rounded-2xl p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <p className="font-sora font-bold text-white flex items-center gap-2">
                          <Trophy size={16} className="text-warning" /> Campus Ambassador Progress
                        </p>
                        <p className="text-sm text-[#8B97B5]">{verifiedCount} / {AMBASSADOR_THRESHOLD}</p>
                      </div>
                      <div className="w-full bg-surface-base rounded-full h-3">
                        <div className="bg-gradient-to-r from-warning to-red-400 h-3 rounded-full transition-all duration-700"
                          style={{ width: `${ambassadorProgress}%` }} />
                      </div>
                      <p className="text-xs text-[#8B97B5]">Reach 100 verified referrals for a special Campus Ambassador badge — on top of the 5 minutes you already earn per referral.</p>
                    </div>
                  )}

                  <div className="bg-surface-elevated border border-white/5 rounded-2xl p-6">
                    <p className="font-sora font-bold text-white text-lg mb-3">How it works</p>
                    <div className="space-y-2 text-sm text-[#8B97B5]">
                      <p>• Share your link — a friend signs up and starts using STUDIA</p>
                      <p>• Once verified, <span className="text-white font-medium">you both get 5 AI minutes</span>, automatically</p>
                      <p>• No limit — every verified referral earns the same 5 minutes, every time</p>
                    </div>
                  </div>
