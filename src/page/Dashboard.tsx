...
import {
  loadAccess, isUnlimitedPlan, totalMinutesAvailable, subscriptionMinutesRemaining,
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <div className="relative w-14 h-14">
        ...
        <div className="absolute inset-0 flex items-center justify-center"><span className={`font-sora font-bold text-sm leading-none ${textColor}`}>{value}</span></div>
      </div>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const avg = quizResults.length > 0 ? Math.round(quizResults.reduce((s, q) => s + (q.total > 0 ? (q.score / q.total) * 100 : 0), 0) / quizResults.length) : 0
      ...
      if (a.planLocked && (a.purchasedMinutesRemaining || 0) <= 0) {
      ...
      if (!localStorage.getItem('studia_onboarded')) setTimeout(() => setShowOnboarding(true), 800)
  ...
  const isMinutesPlan = ['achiever', 'achiever-plus'].includes(plan)
  const isSubscription = ['excellence', 'valedictorian'].includes(plan) && access.subscriptionStatus === 'active'
  const explorerLeft = Math.max(0, 3 - (access.freeCreditsUsed || 0))
  const minutesLeft = totalMinutesAvailable(access)
  const isLocked = access.planLocked && (access.purchasedMinutesRemaining || 0) <= 0 && !isSubscription

  const usageBar = (() => {
    if (isExplorer) {
      const used = access.freeCreditsUsed || 0
      return { used, total: 3, pct: Math.round((used / 3) * 100), color: used >= 3 ? 'bg-red-500' : used >= 2 ? 'bg-yellow-400' : 'bg-mint' }
    }
    if (isSubscription) {
      const used = access.minutesUsed || 0
      const total = access.minutesAllowance || 0
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-surface-light to-white">
      {showOnboarding && <OnboardingModal firstName={firstName} onComplete={() => setShowOnboarding(false)} />}
      ...
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {isAdmin && (
              <button onClick={() => navigate('/admin')} className="flex items-center gap-1.5 bg-warning/10 border border-warning/30 text-warning px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-warning/20 transition ml-1">
          ...
          <div className="flex items-center gap-1 sm:gap-3">
            ...
            <button onClick={handleSignOut} className="flex items-center gap-1.5 text-navy hover:text-indigo-premium transition pl-2 sm:pl-3 border-l border-gray-200 ml-1">
              <LogOut size={18} /><span className="text-sm font-medium hidden sm:inline">Sign out</span>
            </button>
          </div>
        </div>
      </nav>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 sm:space-y-10">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm sm:text-base">
                {stats.streak > 0 ? (<>On a <span className="font-bold text-indigo-premium">{stats.streak}-day streak</span> — keep going!</>) : 'Record a lecture or take a quiz to start your streak.'}
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 px-6 py-5">
              <div className="flex items-center justify-around gap-2">
                {circleStats.map((s, i) => (
                  <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.08 }}><CircleStat {...s} /></motion.div>
                ))}
              </div>
            </div>
            ...
            <div
              className={`rounded-2xl p-5 sm:p-6 border-2 cursor-pointer transition-colors ${
                ...
                : isMinutesPlan ? 'bg-blue-50/50 border-light-blue/30'
                : 'bg-gray-50 border-gray-200'
              }`}
              onClick={() => {
                if (isLocked) { setUpgradeReason('explorer_locked'); setShowUpgradeModal(true) }
                else if (isSubscription && subscriptionMinutesRemaining(access) === 0 && (access.purchasedMinutesRemaining || 0) === 0) { setUpgradeReason('no_minutes_left'); setShowUpgradeModal(true) }
              }}
            >
              <div className="flex items-start justify-between gap-4 flex-wrap">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {isSubscription && <span className="text-[10px] bg-mint/20 text-mint px-2 py-0.5 rounded-full font-semibold">ACTIVE</span>}
                  ...
                  {isLocked ? (
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-sm text-red-700">All 3 free lectures used. Buy minutes or subscribe to continue.</p>
                    </div>
                  ) : isExplorer ? (
                    <div className="space-y-2">
                      <p className="text-sm text-gray-600"><span className="font-bold text-navy">{explorerLeft}</span> of 3 free lectures remaining · lifetime, no reset</p>
                    ...
                    <p className="text-sm text-gray-600"><span className="font-bold text-navy">{Math.round(minutesLeft)}</span> AI minutes remaining</p>
                  ) : isSubscription && usageBar ? (
                    <div className="space-y-2">
                      <p className="text-sm text-gray-600">
                        <span className="font-bold text-navy">{Math.round(usageBar.total - usageBar.used)}</span> of {usageBar.total} minutes remaining
                ...
                {(isLocked || (isSubscription && subscriptionMinutesRemaining(access) <= 30)) && (
                  <button onClick={e => { e.stopPropagation(); setUpgradeReason(isLocked ? 'explorer_locked' : 'no_minutes_left'); setShowUpgradeModal(true) }}
            ...
            <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border-2 border-purple-500/20 rounded-2xl p-5 sm:p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-600 text-sm">Invite classmates and earn up to <span className="font-semibold text-purple-600">150+ bonus credits</span>. They get 2 too.</p>
                </div>
                ...
                {cards.map((card, i) => (
                  <motion.button key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                    ...
                    <div className={`bg-gradient-to-br ${card.color} to-transparent rounded-2xl p-4 sm:p-5 border border-gray-200 hover:border-indigo-premium/50 hover:shadow-lg transition h-full ${isLocked && card.path !== '/payments' ? 'opacity-60' : ''}`}>
                      <div className="w-10 h-10 rounded-lg bg-white/20 flex items-center justify-center text-white mb-3 group-hover:scale-110 transition"><card.icon size={20} /></div>
                      ...
                      <div className="hidden sm:flex items-center gap-1 text-indigo-premium text-xs font-medium opacity-0 group-hover:opacity-100 transition mt-2">Open <ChevronRight size={12} /></div>
                    </div>
                  </motion.button>
                ))}
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
              ...
              <div className="relative z-10">
                ...
                <p className="text-white/90 mb-6 max-w-2xl text-sm sm:text-base">Record your lectures, let SAGE generate Smart Ink notes, then subscribe to ask SAGE to quiz you, explain anything, or check your progress.</p>
                <button onClick={() => navigate('/pricing')} className="bg-white text-indigo-premium px-6 py-3 rounded-lg font-semibold hover:bg-gray-100 transition text-sm sm:text-base">See Plans — from KSh 45</button>
              </div>
            </motion.div>
