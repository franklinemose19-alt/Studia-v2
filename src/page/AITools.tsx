...
import {
  ...
  Zap, FileText, BookOpen, ChevronDown, ChevronUp, Check
} from 'lucide-react'
      ...
      <div className="space-y-3">
        ...
        {r.questions.map((q, i) => (
          <div key={i} className="bg-surface-base border border-white/10 rounded-2xl overflow-hidden">
            <button
              onClick={() => setExpandedQ(expandedQ === `q${i}` ? null : `q${i}`)}
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bg-surface-base border border-white/10 rounded-2xl p-5">
          ...
          <ul className="space-y-2">
            {r.common_themes.map((t, i) => (
              <li key={i} className="text-sm text-[#8B97B5] flex items-start gap-2">
        ...
        <div className="bg-surface-base border border-white/10 rounded-2xl p-5">
          ...
          <ul className="space-y-2">
            {r.exam_tips.map((t, i) => (
              <li key={i} className="text-sm text-[#8B97B5] flex items-start gap-2">
                <span className="text-brand-blue">•</span> {t}
              </li>
            ))}
          </ul>
        </div>
    ...
    <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {section.key_terms.map((term, j) => (
                    <span key={j} className="text-xs bg-purple-500/10 text-purple-400 border border-purple-500/20 px-3 py-1 rounded-full">
                      {term}
                    </span>
                  ))}
                </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-surface-base border border-white/10 rounded-2xl p-5">
          ...
          <ul className="space-y-2">
            {r.revision_checklist.map((item, i) => (
              <li key={i} className="text-sm text-[#8B97B5] flex items-start gap-2">
        ...
        {showCamera && (
          ...
          >
            ...
            <div className="flex gap-4">
              <button onClick={capturePhoto} className="bg-brand-blue text-white px-8 py-3 rounded-xl font-medium hover:bg-brand-blue/90">
                📸 Capture
              </button>
              <button onClick={stopCamera} className="bg-white/10 text-white px-8 py-3 rounded-xl font-medium hover:bg-white/20">
                Cancel
              </button>
            </div>
          </motion.div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {tabs.map((t) => {
              ...
              return (
                <button
                  key={t.id}
                  onClick={() => { setTab(t.id); clearAll() }}
                  className={`p-4 rounded-2xl border text-left transition-all ${
                    tab === t.id
                      ? 'bg-brand-blue/10 border-brand-blue/40 text-white'
                      : 'bg-surface-elevated border-white/5 text-[#8B97B5] hover:border-white/20'
                  }`}
                >
            ...
            {image ? (
              ...
              <div className="border border-white/10 border-dashed rounded-xl p-8 flex flex-col items-center justify-center gap-4 text-[#4A5568]">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={startCamera}
                    className="flex items-center gap-2 bg-surface-base border border-white/10 text-white px-4 py-2 rounded-xl hover:border-brand-blue/40 text-sm transition-colors"
                  >
            ...
            <textarea
              ...
              placeholder={
                tab === 'snapsolve'
                  ...
                  : tab === 'pastpapers'
                  ? 'Paste past paper questions here...'
                  : 'Paste your notes here to expand into deep study material...'
              }
            ...
            >
              {loading ? (
                <>
                  ...
                  {tab === 'snapsolve' ? 'Solving...' : tab === 'pastpapers' ? 'Analyzing paper...' : 'Generating deep notes...'}
                </>
              ) : (
                <>
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={copyResult}
                      className="flex items-center gap-2 bg-surface-elevated border border-white/10 text-[#8B97B5] px-4 py-2 rounded-xl hover:text-white text-sm transition-colors"
                    >
                      <Copy size={15} />
                      {copied ? 'Copied!' : 'Copy'}
                    </button>
