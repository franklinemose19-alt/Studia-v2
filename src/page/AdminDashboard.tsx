...
interface AdminStats {
  ...
  aiInfrastructure: { openai: AIProviderInfo }
}

const PLAN_ICONS: Record<string, string> = { explorer: '🌍', achiever: '🎯', 'achiever-plus': '🎯', excellence: '🚀', valedictorian: '🏆', none: '👤' }
const PLAN_COLORS: Record<string, string> = { explorer: 'text-gray-400', achiever: 'text-light-blue', 'achiever-plus': 'text-light-blue', excellence: 'text-mint', valedictorian: 'text-warning', none: 'text-[#8B97B5]' }
const PLAN_STROKE: Record<string, string> = { explorer: '#9CA3AF', achiever: '#60A5FA', 'achiever-plus': '#60A5FA', excellence: '#2EE59D', valedictorian: '#F59E0B', none: '#8B97B5' }
...
function ProviderCard({ info, onToggle, onBudgetSave }: {
  info: AIProviderInfo
...
export default function AdminDashboard() {
  ...
  const toggleAIProvider = async (enabled: boolean) => {
    if (!userId) return
    try {
      const res = await fetch('/api/admin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminUserId: userId, action: 'toggle_ai_provider', enabled }) })
      if (res.ok) { toast.success(`OpenAI ${enabled ? 'enabled' : 'disabled'}`); fetchStats() }
      else toast.error('Failed to update provider')
    } catch { toast.error('Connection error') }
  }

  const saveAIBudget = async (monthlyBudgetUsd: number, safetyThresholdPct: number) => {
    if (!userId) return
    try {
      const res = await fetch('/api/admin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ adminUserId: userId, action: 'update_ai_budget', monthlyBudgetUsd, safetyThresholdPct }) })
  ...
  return (
    <div className="min-h-screen bg-surface-base">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="bg-surface-elevated border border-white/5 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-5"><Zap size={15} className="text-red-400" /><p className="text-sm font-semibold text-white">API Costs</p></div>
        ...
        <div className="bg-surface-elevated border border-white/5 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-4"><Server size={15} className="text-brand-blue" /><p className="text-sm font-semibold text-white">AI Provider</p></div>
          <ProviderCard info={s.aiInfrastructure.openai} onToggle={toggleAIProvider} onBudgetSave={saveAIBudget} />
        </div>
