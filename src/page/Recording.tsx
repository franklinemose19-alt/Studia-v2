...
import { Mic, Square, Play, Pause, Trash2, Download, ArrowLeft } from 'lucide-react'
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          <div>
            ...
            <p className="text-[#8B97B5]">Start recording your lecture. We'll transcribe and summarize it automatically.</p>
          </div>
          ...
          <div className="bg-surface-elevated border border-white/5 rounded-3xl p-8 space-y-6">
            <div className="space-y-4">
              <canvas
                ...
                width={500}
                height={200}
                className="w-full h-48 bg-surface-base rounded-xl"
              />
              
              <div className="text-center">
                <p className="text-5xl font-mono font-bold text-brand-blue mb-4">{formatTime(duration)}</p>
                <p className="text-sm text-[#8B97B5]">
                  {isRecording ? 'Recording in progress...' : recordings.length > 0 ? `${recordings.length} recording(s) saved` : 'Ready to record'}
                </p>
              </div>
              ...
              <div className="flex gap-4 justify-center">
                {!isRecording ? (
                  <button
                    ...
                    className="inline-flex items-center gap-3 bg-brand-blue text-white font-semibold px-8 py-4 rounded-2xl text-lg hover:bg-brand-blue/90 transition-all"
                  >
                    <Mic size={24} />
                    Start Recording
                  </button>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-3 bg-red-500 text-white font-semibold px-8 py-4 rounded-2xl text-lg hover:bg-red-600 transition-all animate-pulse"
                  >
                    <Square size={24} />
                    Stop Recording
                  </button>
              ...
              <div className="space-y-3">
                {recordings.map((recording) => (
                  <motion.div
                    ...
                    className="bg-surface-elevated border border-white/5 rounded-2xl p-4 flex items-center justify-between"
                  >
