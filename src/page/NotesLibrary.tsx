...
export default function NotesLibrary() {
  ...
  const addManualNote = () => {
    ...
    const note: Note = {
      id: `note-${Date.now()}`, title: noteTitle.trim(), content: noteContent.trim(),
      course: noteCourse.trim() || undefined, date: new Date().toLocaleDateString('en-KE'), source: 'manual',
    }
    const updated = [note, ...notes]
    setNotes(updated); saveNotes(updated)
    setNoteTitle(''); setNoteContent(''); setNoteCourse('')
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      toast.error('Failed to generate summary. Check your connection.')
    } finally {
  ...
  const saveSummaryAsNote = () => {
    if (!summary) return
    const note: Note = {
      id: `note-${Date.now()}`, title: `AI Summary — ${new Date().toLocaleDateString('en-KE')}`,
      content: summary, date: new Date().toLocaleDateString('en-KE'), source: 'ai',
    }
    const updated = [note, ...notes]
    setNotes(updated); saveNotes(updated)
    setActiveTab('notes')
              ...
              {showAddNote && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                  ...
                  <div>
                    ...
                    <textarea value={noteContent} onChange={e => setNoteContent(e.target.value)} placeholder="Write your notes here..."
              ...
              <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-12 text-center">
                ...
                {!search && (
                  <div className="flex justify-center gap-3 flex-wrap">
                    <button onClick={() => setShowAddNote(true)} className="bg-indigo-premium text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-purple-premium transition">Write a Note</button>
                    <button onClick={() => navigate('/recording')} className="border border-gray-200 text-navy px-5 py-2.5 rounded-xl font-semibold text-sm hover:bg-gray-50 transition">Record a Lecture</button>
                  </div>
                )}
              </div>
            ) : (
                ...
                {filteredNotes.map((note, i) => (
                  <motion.div key={note.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <div className="px-5 py-4 flex items-start justify-between gap-3">
                      <button onClick={() => setExpandedNote(expandedNote === note.id ? null : note.id)} className="flex-1 text-left min-w-0">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button onClick={() => deleteNote(note.id)} className="p-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition"><Trash2 size={15} /></button>
                        {expandedNote === note.id ? <ChevronUp size={16} className="text-gray-400" /> : <ChevronDown size={16} className="text-gray-400" />}
                      </div>
                    </div>
                        ...
                        <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden border-t border-gray-100">
                          <div className="px-5 py-4">
                            ...
                            <div className="flex gap-2 mt-4 flex-wrap">
                              <button onClick={() => navigate('/sage')} className="flex items-center gap-1.5 bg-indigo-premium/10 text-indigo-premium px-4 py-2 rounded-xl text-xs font-semibold hover:bg-indigo-premium/20 transition">🧠 Ask SAGE</button>
                              <button onClick={() => downloadNoteAsPDF(note)} className="flex items-center gap-1.5 border border-gray-200 text-navy px-4 py-2 rounded-xl text-xs font-semibold hover:bg-gray-50 transition"><FileText size={13} /> Download PDF</button>
                              <button onClick={() => downloadNote(note)} className="flex items-center gap-1.5 border border-gray-200 text-navy px-4 py-2 rounded-xl text-xs font-semibold hover:bg-gray-50 transition"><Download size={13} /> Download Text</button>
                            </div>
                          </div>
                        </motion.div>
                      )}
          ...
          <div className="space-y-5">
            <div>
              ...
              <p className="text-gray-500 text-sm">
                ...
                {!isUnlimitedPlan(access) && remaining > 0 && <span className="text-indigo-premium ml-2">· {remaining} free credit{remaining !== 1 ? 's' : ''} left</span>}
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-4">
                  ...
                  {summarizeImage ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => { setSummarizeImage(null); setSummarizeImageName('') }} className="text-gray-400 hover:text-red-500 transition"><X size={18} /></button>
                      </div>
                    </div>
                  ) : (
              ...
              <button onClick={handleSummarize} disabled={summarizeLoading}
                ...
                {summarizeLoading ? (<><Loader size={20} className="animate-spin" /> Summarizing...</>) : (<><Sparkles size={20} /> Generate Summary</>)}
              </button>
            </div>

            <AnimatePresence>
              {summary && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-indigo-premium/30 p-6 space-y-4">
                  ...
                  <div className="flex gap-3 flex-wrap">
                    <button onClick={saveSummaryAsNote} className="flex items-center gap-1.5 bg-indigo-premium text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-purple-premium transition">
                      <Plus size={15} /> Save to Notes
                    </button>
                    <button onClick={() => setSummary('')} className="flex items-center gap-1.5 border border-gray-200 text-navy px-4 py-2 rounded-xl text-sm font-semibold hover:bg-gray-50 transition">
                      <X size={15} /> Clear
                    </button>
                    <button onClick={() => navigate('/sage')} className="flex items-center gap-1.5 bg-gradient-to-r from-brand-blue/10 to-purple-500/10 text-indigo-premium px-4 py-2 rounded-xl text-sm font-semibold hover:opacity-80 transition">
                      🧠 Ask SAGE more
                    </button>
                  </div>
                </motion.div>
